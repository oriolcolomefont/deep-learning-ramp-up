...
class LinearRegressionSGD(nn.Module):
    ...
    def __init__(
        ...
        self.epochs = epochs
        self.linear = nn.Linear(input_dim, 1)  # Updated to handle vector inputs
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return self.linear(x)  # No need to reshape as input is already a vector
...
if __name__ == "__main__":
    ...
    model = LinearRegressionSGD(
        learning_rate=0.01, batch_size=32, epochs=1000, input_dim=1
    )
    model.fit(x_train, y_train, x_val, y_val)

    # Example prediction with vector input
    x_new = torch.tensor([[5.0]], dtype=torch.float32)  # Example with single feature
